...
from bam_masterdata.openbis import OpenbisEntities, SyncAction, SyncResult, SyncTarget
...
class ObjectType(BaseEntity):
    ...
    def _openbis_data_type(self, prop: PropertyTypeAssignment) -> str:
        """Translates bam-masterdata data types to their openBIS representation.

        Args:
            prop (PropertyTypeAssignment): The property to translate its data type.

        Returns:
            str: The value string as defined in `DataType`.
        """
        if prop.data_type in (DataType.OBJECT, DataType.SAMPLE):
            return DataType.SAMPLE.value
    ...
    def _create_object_type(self, openbis: "Openbis", result: SyncResult) -> None:
        ...
        self._sync_property_assignments(
            obj_type=obj_type, openbis=openbis, result=result
        )

    def _sync_object_type_definition(
        self,
        obj_type: "SampleType",
        result: SyncResult,
    ) -> None:
        ...
        if obj_type.generatedCodePrefix != self.defs.generated_code_prefix:
            result.rejected(
                target=SyncTarget.OBJECT_TYPE,
                code=self.defs.code,
                field="generated_code_prefix",
                old_value=obj_type.generatedCodePrefix,
    ...
    def _update_existing_property(
        ...
        if prop.code.startswith("$"):
            result.add(
                ...
                field="type_and_assignment",
                old_value=None,
                new_value=None,
                message="Internal property types and assignments cannot be modified by the current user.",
            )
            return None

        # --- PropertyType fields ---
        property_type = openbis.get_property_type(prop.code)
        expected_data_type = self._openbis_data_type(prop)
        # Unsupported: data type changes
        ...
        # Unsupported: vocabulary changes
        if prop.data_type == DataType.CONTROLLEDVOCABULARY:
            current_vocabulary = getattr(property_type, "vocabulary", None)

            if current_vocabulary != prop.vocabulary_code:
                result.rejected(
                    target=SyncTarget.PROPERTY_TYPE,
                    code=prop.code,
                    field="vocabulary_code",
                    old_value=current_vocabulary,
                    new_value=prop.vocabulary_code,
                )
                return None

        property_type_modified = False

        # Supported: label
        if property_type.label != prop.property_label:
            old_value = property_type.label
            property_type.label = prop.property_label
            property_type_modified = True
        ...
        if property_type_modified:
            property_type.save()

        # --- PropertyAssignment fields ---

        assignments = obj_type.get_property_assignments()
        assignment = next(
            assignment for assignment in assignments if assignment.code == prop.code
        )

        assignment_modified = False

        if assignment.section != prop.section:
            result.modified(
                target=SyncTarget.PROPERTY_ASSIGNMENT,
                code=prop.code,
                parent_code=self.defs.code,
                field="section",
                old_value=assignment.section,
                new_value=prop.section,
            )
            assignment_modified = True

        if assignment.mandatory != prop.mandatory:
            result.modified(
                target=SyncTarget.PROPERTY_ASSIGNMENT,
                code=prop.code,
                parent_code=self.defs.code,
                field="mandatory",
                old_value=assignment.mandatory,
                new_value=prop.mandatory,
            )
            assignment_modified = True

        if assignment.showInEditView != prop.show_in_edit_views:
            result.modified(
                target=SyncTarget.PROPERTY_ASSIGNMENT,
                code=prop.code,
                parent_code=self.defs.code,
                field="show_in_edit_views",
                old_value=assignment.showInEditView,
                new_value=prop.show_in_edit_views,
            )
            assignment_modified = True

        if assignment.ordinal != prop.ordinal:
            result.add(
                action=SyncAction.IGNORED,
                target=SyncTarget.PROPERTY_ASSIGNMENT,
                code=prop.code,
                parent_code=self.defs.code,
                message="Modification of the ordinal attribute in property assignments is not supported yet.",
            )
    ...
    def _create_property_assignment(
        self,
        prop: PropertyTypeAssignment,
        obj_type: "SampleType",
        openbis: "Openbis",
        ...
        try:
            ...
            kwargs = {
                ...
                "label": prop.property_label,
                "description": prop.description,
                "dataType": self._openbis_data_type(prop),
            }
            # For CONTROLLEDVOCABULARY properties with a defined vocabulary_code, we need to create the property with the vocabulary assigned
            if prop.data_type == DataType.CONTROLLEDVOCABULARY and prop.vocabulary_code:
                kwargs["vocabulary"] = prop.vocabulary_code

            openbis.new_property_type(**kwargs).save()
            result.created(
                target=SyncTarget.PROPERTY_TYPE,
                code=prop.code,
            )
    ...
    def _sync_property_assignments(
        self,
        obj_type: "SampleType",
        openbis: "Openbis",
        result: SyncResult,
        ...
        # BAM Masterdata property assigned codes
        model_props = {prop.code: prop for prop in self.properties}
        ...
        for code in new_codes:
            self._create_property_assignment(
        ...
        # Ignoring deletion of assigned properties
        for code in missing_codes:
            result.add(
                ...
                code=code,
                parent_code=self.defs.code,
                message="Deletion of property assignments is not implemented yet.",
            )
    ...
    def to_openbis_sync(self, openbis: "Openbis") -> SyncResult:
        ...
        - add new assigned properties to existing object types
        - modify specific fields in object types or assigned properties

        Not supported operations:
        - deletion of object types
        - deletion of previously assigned properties
        - assigned properties code changes
        - assigned properties unit changes
        - assigned properties data type changes
        ...
        if self.defs.code not in openbis_object_types:
            self._create_object_type(openbis=openbis, result=result)
            return result

        obj_type = openbis.get_object_type(self.defs.code)

        # Otherwise, synchronize definition
        self._sync_object_type_definition(obj_type=obj_type, result=result)
        # Then synchronize properties
        self._sync_property_assignments(
            obj_type=obj_type, openbis=openbis, result=result
        )
