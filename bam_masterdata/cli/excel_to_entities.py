...
VALIDATION_RULES = {
    "SAMPLE_TYPE": {
        ...
        "Description": {"key": "description", "pattern": r".*", "is_description": True},
        "Validation script": {
    ...
    "PROPERTY_TYPE": {
        ...
        "Vocabulary code": {"key": "vocabularyCode", "pattern": r"^\$?[A-Za-z0-9_.]+$"},
...
def extract_value(
    ...
    error_message = (
        f"Invalid value '{value}' at row {row}, column {column} in sheet {sheet.title}"
    )
...
def process_entity(
    sheet, start_index_row, header_terms, expected_terms, validation_rules
):
    attributes = {}

    for term in expected_terms:
        if term not in header_terms:
            logger.error(f"{term} not found in the headers.")
        else:
            term_index = header_terms.index(term)
            cell = sheet.cell(row=start_index_row + 2, column=term_index + 1)
            cell_value = extract_value(
                sheet,
                start_index_row + 2,
                term_index + 1,
                validation_rules[term].get("pattern"),
            )

            # Handle boolean conversion
            if validation_rules[term].get("is_bool"):
                cell_value = str_to_bool(
                    value=cell_value,
                    term=term,
            ...
            elif validation_rules[term].get("is_data"):
                if cell_value not in DATA_TYPES:
                    logger.error(
            ...
            elif validation_rules[term].get("extra_validation") == "is_reduced_version":
            ...
            elif validation_rules[term].get("allow_empty") and not cell_value:
                cell_value = ""

            # Handle URL template validation (allows empty but must be a valid URL)
            elif validation_rules[term].get("is_url") and cell_value:
                url_pattern = validation_rules[term]["pattern"]
                if not re.match(url_pattern, str(cell_value)):
                    logger.error(
                        f"Invalid URL format: {cell_value} in {cell.coordinate} (Sheet: {sheet.title})"
                    )

            attributes[validation_rules[term]["key"]] = cell_value
...
def block_to_entity_dict(
    ...
    if entity_type not in VALIDATION_RULES:
    ...
    attributes_dict = process_entity(
        sheet,
        start_index_row,
        header_terms,
        list(VALIDATION_RULES[entity_type].keys()),
        VALIDATION_RULES[entity_type],
    )

    # Extract additional attributes if necessary
    if entity_type in {"SAMPLE_TYPE", "OBJECT_TYPE", "EXPERIMENT_TYPE", "DATASET_TYPE"}:
        attributes_dict["properties"] = (
            properties_to_dict(sheet, start_index_row, last_non_empty_row) or {}
        )

    elif entity_type == "VOCABULARY_TYPE":
        attributes_dict["terms"] = (
            terms_to_dict(sheet, start_index_row, last_non_empty_row) or {}
        )
