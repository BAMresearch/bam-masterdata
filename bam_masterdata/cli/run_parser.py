...
class RunParser:
    def __init__(
        ...
        self.files_parser = files_parser or {}
    ...
    def run_parser(self) -> None:
        ...
        if self.collection_type not in ["COLLECTION", "DEFAULT_EXPERIMENT"]:
            logger.error(
                f"Invalid collection_type '{self.collection_type}'. Must be either 'COLLECTION' or 'DEFAULT_EXPERIMENT'."
            )
            return

        # Specify the space
        try:
            space = self.openbis.get_space(self.space_name)
        except Exception:
            space = None
        ...
        if self.project_name.upper() in [p.code for p in space.get_projects()]:
            project = space.get_project(self.project_name)
        else:
        ...
        if not self.collection_name:
            ...
            if self.collection_name.upper() in [
                c.code for c in project.get_collections()
            ]:
                ...
                collection_openbis = self.openbis.new_collection(
        ...
        for object_id, object_instance in collection.attached_objects.items():
                ...
                if property_metadata.data_type == "OBJECT":
                    if isinstance(value, str):
                        ...
                        if not referenced_identifier:
                            ...
                            if not self.collection_name:
                                referenced_identifier = f"/{self.space_name}/{self.project_name}/{value.code}"
                            else:
                                referenced_identifier = f"/{self.space_name}/{self.project_name}/{self.collection_name}/{value.code}"
            ...
            if not object_instance.code:
                ...
                identifier = (
                    f"/{self.space_name}/{self.project_name}/{object_instance.code}"
                    if not self.collection_name
                    else f"/{self.space_name}/{self.project_name}/{self.collection_name}/{object_instance.code}"
                )
                try:
        ...
        for files in self.files_parser.values():
            try:
                ...
                continue
            logger.info(f"Files uploaded to openBIS collection {self.collection_name}.")
        ...
        for parent_id, child_id in collection.relationships.values():
            if parent_id in openbis_id_map and child_id in openbis_id_map:
                ...
                child_openbis = self.openbis.get_object(child_openbis_id)
                child_openbis.add_parents(parent_openbis_id)
                child_openbis.save()

                logger.info(
                    f"Linked child {child_openbis_id} to parent {parent_openbis_id} in collection {self.collection_name}."
                )

    def run_parser_withthreading(self, timeout: int = 300, max_workers: int = 1) -> int:
