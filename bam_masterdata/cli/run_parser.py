...
def run_parser_with_transactions(
    ...
    # Start a transaction for creating objects in openBIS
    object_transaction = openbis.new_transaction()

    # Map the objects added to CollectionType to objects in openBIS using pyBIS
    openbis_id_map = {}
    object_codes = set()  # to track object codes and detect duplicates
    for object_id, object_instance in collection.attached_objects.items():
        # Map PropertyTypeAssignment to pybis props dictionary
        obj_props = {}
        for key in object_instance._properties.keys():
        ...
        if not object_instance.code:
            ...
            object_transaction.add(object_openbis)
        else:
            identifier = (
                f"/{space_name}/{project_name}/{object_instance.code}"
                if not collection_name
        ...
        # detect duplicates
        if object_instance.code in object_codes:
            logger.warning(
                f"Object with local ID {object_instance.code} already has an openBIS mapping, overwriting with latest object identifier."
            )
        object_codes.add(object_instance.code)
